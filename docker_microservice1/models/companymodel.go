...
// GetCompanies returns companies list
func (a *API) GetCompaniesMongoRpc(db *mongo.Client, reply *[]bson.M) error {
	opt := options.FindOptions{}

	collection := db.Database(os.Getenv("DBNAMEMONGO")).Collection("companies1")

	cursor, err := collection.Find(context.TODO(), bson.D{{Key: "name", Value: ""}}, &opt)
	var companies *[]bson.M
	...
	if len(*companies) <= 0 {
		returnerr := errors.New(lang.Get("no_result"))
		*reply = *companies
		return returnerr
	}
	*reply = *companies
